      ...
      <div className="relative w-full h-full bg-navy-light/50 backdrop-blur-md rounded-2xl border border-accent-yellow/30 shadow-glow-yellow flex flex-col overflow-hidden">
        <div className="p-6 border-b border-slate-dark/30 flex flex-col items-center">
            <div className="relative w-24 h-24 mb-3">
                ...
                <img src="https://i.imgur.com/3B0k6f8.png" alt="AI Assistant Avatar" className="w-full h-full rounded-full object-cover border-2 border-accent-yellow/50" />
            </div>
        ...
        <div className="flex-grow p-4 overflow-y-auto space-y-4">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex items-start gap-2.5 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              {msg.sender === 'ai' && <img className="w-8 h-8 rounded-full" src="https://i.imgur.com/3B0k6f8.png" alt="AI Avatar"/>}
